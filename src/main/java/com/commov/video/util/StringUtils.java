...
public class StringUtils {
	...
	/**
	 * 生成邀请码 16位 CMYQ YYYY MM XXXXXX
	 * 
	 * @return String
	 */
	public static String createInviteCode() {
		StringBuilder code = new StringBuilder();
		code.append("CMYQ").append(CalendarUtil.getYear()).append(
				String.format("%1$,02d", CalendarUtil.getMonth()));
		for (int i = 0; i < INVITE_CODE_LENGTH - 10; i++) {
			code.append(COUPONCODE_TABLE[(int) (COUPONCODE_TABLE.length * Math
					.random())]);
		}
		return code.toString();
	...
	public static boolean isMobile(final String buf) {
		...
		// else if (buf.length() == 8) { // 小灵通的号是8位，但目前短信通道不支持小灵通
		// return true;
